...
/***/ function(module, exports, __webpack_require__) {
	...
	var Biomorph = __webpack_require__(1)
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function() {
		...
		function Biomorph (ctx, width, height, genes) {
			...
			this.generatedCentre = _.cloneDeep(this.workingCentre)
			this.origin = _.cloneDeep(this.workingCentre)
		...
		Biomorph.prototype.randomize = function () {
			...
			_.extend(this, Biomorph.randomGenes())
			this.generate()
		}

		Biomorph.prototype.resetCentre = function () {

			this.generatedCentre = _.cloneDeep(this.origin)
			this.workingCentre = _.cloneDeep(this.origin)
		}
		...
		Biomorph.randomGenes = function () {
			...
			return {
				genes: [_.random(-20, 20), _.random(-20, 20), _.random(10, 20), _.random(-10, 10), _.random(-10, 10), _.random(10, 20), _.random(10, 20), _.random(10, 20), _.random(3, 6)],
				dGenes: [_.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), _.sample(Biomorph.swellTypes), "Same", _.sample(Biomorph.swellTypes)],
				segNoGene: _.random(1, 6),
				segDistGene: _.random(-30, 30),
				symmetrical: true,
				spokesGene: _.sample(Biomorph.spokesGenes),
				trickleGene: 1,
				mutSizeGene: 5,
				mutProbGene: 10
			}
		...
		Biomorph.prototype.getGenes = function () {
			return _.cloneDeep(_.pick(this, ['genes', 'dGenes', 'trickleGene', 'mutProbGene', 'mutSizeGene', 'spokesGene', 'symmetrical', 'segNoGene', 'segDistGene']))
		}
		...
		Biomorph.prototype.breed = function () {

			// A child starts with a copy of the genes of its parent
			var child = this.getGenes()

			if (Biomorph.mutations.mutationRate && _.random(0, 100) < child.mutProbGene) {

				child.mutProbGene += _.sample([-1, 1])
			...
			for (var i = 0; i < child.genes.length - 1; i++) {
				...
				if (_.random(0, 100) < child.mutProbGene) {
					child.genes[i] += child.mutSizeGene * _.sample([-1, 1])
				}
			}

			if (_.random(0, 100) < child.mutProbGene) {
				child.genes[child.genes.length - 1] += _.sample([-1, 1])
			}
			...
			if (Biomorph.mutations.segmentation && _.random(0, 100) < child.mutProbGene) {
				child.segNoGene += _.sample([-1, 1])
			}
			...
			if (Biomorph.mutations.gradient && child.segNoGene > 1) {
				...
				for (var i = 0; i < child.dGenes.length - 2; i++) {
					...
					if (_.random(0, 100) < child.mutProbGene/2) {
			...
			if (Biomorph.mutations.taperingTwigs && Biomorph.mutations.fatten && _.random(0, 100) < child.mutProbGene) {
				child.dGenes[child.dGenes.length - 2] = Biomorph.nextSwell(child.dGenes[child.dGenes.length - 2])
			}

			if (Biomorph.mutations.segmentation && child.segNoGene > 1 && _.random(0, 100) < child.mutProbGene) {
				child.segDistGene += _.sample([-1, 1])
			}

			if (Biomorph.mutations.asymmetry && _.random(0, 100) < child.mutProbGene/2) {
				child.symmetrical = !child.symmetrical
			}

			if (Biomorph.mutations.radialSym && _.random(0, 100) < child.mutProbGene/2) {
			...
			if (Biomorph.mutations.scalingFactor && _.random(0, 100) < child.mutProbGene) {
				child.trickleGene += _.sample([-1, 1])
			...
			if (Biomorph.mutations.mutationSize && _.random(0, 100) < child.mutProbGene) {
				child.mutSizeGene += _.sample([-1, 1])
			...
			this.children.push(new Biomorph(this.ctx, this.canvasWidth, this.canvasHeight, child))
